...
public class ApplicationPersistenceContext : IApplicationPersistenceContext
{
    ...
    private void Save<TSettings>(string subDirectoryPath, string fileName, TSettings settings)
    {
    ...
    {
        Save(string.Empty, "settings", appSettings);
    }

    public void Save(GameSettings gameSettings)
    {
        Save(gameSettings.Game.ToString(), "settings", gameSettings);
    }

    public void Save(ModSettings modSettings)
    {
        Save(
            modSettings.Game.ToString(),
            Path.GetFileNameWithoutExtension(modSettings.FileName),
            modSettings
        );
    }
    ...
    {
        var path = $"{DirectoryPath.SettingsDirectory}\\{mod.Game.ToString()}\\{Path.GetFileNameWithoutExtension(mod.FileName)}.json";
